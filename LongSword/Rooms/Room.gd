...
const ENEMY_SCENES: Dictionary = {
	"FLYING_CREATURE":
		preload("res://Characters/Enemies/Flying Creature/FlyingCreature.tscn"),
	"GOBLIN":
...
func _close_entrance() -> void:
	for entry_position in entrance.get_children():
		tilemap.set_cellv(tilemap.world_to_map(entry_position.position),TOP)
		tilemap.set_cellv(tilemap.world_to_map(entry_position.position) + Vector2.DOWN,DOWN)
func _back_close_entrance() -> void:
	for entry_position in entrance.get_children():
		tilemap.set_cellv(tilemap.world_to_map(entry_position.position),FLOOR_INDEX)
		tilemap.set_cellv(tilemap.world_to_map(entry_position.position) + Vector2.DOWN,FLOOR_INDEX)
